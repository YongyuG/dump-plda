...
%% train plda
plda = gplda_em(data', label, nphi, niter);

%% score plda
[PLDA_scores,U,Uk] = score_gplda_trials(plda, enrol_ivec', test_ivec');
[eer, dcf08, dcf10] = compute_eer(PLDA_scores, labels, true)

% PLDA_scores_col = reshape(PLDA_scores',length(PLDA_scores(:)),1);
% 
% disp(size(PLDA_scores_col))
% disp(size(test_ivec))
% figure;
